...
function App() {
  ...
  const { isLoading, isError, data, error } = useQuery("someId", () =>
    fetch(`http://localhost:4000/ingredients`).then((res) => res.json())
  );
  dispatch({
    type: "pizza/servAnswer",
    isLoading: isLoading,
    isError: isError,
    error: error,
  });
    ...
    <>
      <nav>
        <ul>
          ...
          <li>
            <Link to={auth}>OrderList</Link>
          </li>
          <li>
            <Link to="/payment-form">PaymentForm</Link>
          </li>
        </ul>
      </nav>
